  ...
  return (
    <div className="mx-auto max-w-2xl lg:max-w-5xl">
      ...
      <div className="mt-6 sm:mt-10">
        <h1 className="text-1xl tracking-tight sm:text-3xl text-zinc-900 dark:text-zinc-100">{locale === 'ja' ? '担当領域' : 'Responsibilities'}</h1>
        <p className="mt-6 text-base text-zinc-600 dark:text-zinc-400">
          {locale === 'ja' ? '要件整理 / 技術調査 / 詳細設計 / 実装 / デプロイ / テスト' : 'Requirements Gathering / Research / Detailed Specification / Development / Deployment / Test'}
        </p>
      </div>
      <div className="mt-6 sm:mt-10">
        <h1 className="text-1xl tracking-tight sm:text-3xl text-zinc-900 dark:text-zinc-100">{locale === 'ja' ? 'テストとデバッグ' : 'Testing and Debugging'}</h1>
      ...
      <div className="mt-6 sm:mt-10">
        <h1 className="text-1xl tracking-tight sm:text-3xl text-zinc-900 dark:text-zinc-100">{locale === 'ja' ? '成果' : 'Results and Outcomes'}</h1>
        <p className="mt-6 text-base text-zinc-600 dark:text-zinc-400">
          {locale === 'ja'
            ? 'オフラインコンバージョンを Microsoft Advertising へ送信できるようにしたことで、オンライン広告が電話購入・問い合わせにどの程度寄与したかを可視化できるようになりました。これにより広告効果を総合的に把握し、より精度の高いリターゲティングやターゲティング最適化が可能となり、ROI 向上と効果的なマーケティング戦略の構築に貢献しました。'
